...
#        self.char2id['<unk>'] = 3 # alternative <unk> token
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        char_ids = self.words2charindices(sents) # @return word_ids (list[list[list[int]]]): sentence(s) in indices

        # shape (batch_size, max_sentence_length, max_word_length)
        sents_padded = pad_sents_char(char_ids, self.char_pad)

        # tensor size (max_sentence_length, batch_size, max_word_length)
