...
def question_1f_sanity_check(hiway):
    print("-"*80)
    print("Running Sanity Check for Question 1f: build a highway network")
    print("-"*80)
    x = torch.randn(50,hiway.D_emb)

    print("input size：50，"+str(hiway.D_emb))

    result = hiway.forward(x)

    print("output size :",str(result.shape) )

    assert(result.shape == (50,hiway.D_emb))
    print("Sanity Check Passed for Question 1f:Highway Network")


def question_1g_sanity_check(cnn):
    print("-"*80)
    print("Running Sanity Check for Question 1g: build a cnn network")
    print("-"*80)
    ...
    x = np.random.randn(batch_size,max_word_len,cnn.emb_size)

    print("input size：50，"+str(x.shape))

    result = cnn.forward(x)

    print("output size :",str(result.shape) )

    assert(result.shape == (50,cnn.emb_size))
    print("Sanity Check Passed for Question 1g:cnn network")
