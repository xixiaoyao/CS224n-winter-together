...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device)  # Tensor: (src_len, b, max_w_len)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device)  # Tensor: (tgt_len, b, max_w_len)
