...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # 每个单词为单位，拆分成字符列表
        char_index = self.words2charindices(sents)                           # batch_size, sentence_length, word_length
        # 统一字符列表长度，长度不足用0填充
        pad_word = pad_sents_char(char_index, self['<pad>'])                 # batch_size, max_sentence_length, max_word_length
        sents_var = torch.tensor(pad_word, dtype=torch.long, device=device)
        sents_var = sents_var.permute(1, 0, 2).to(device)                    # max_sentence_length, batch_size, max_word_length
        return sents_var
