...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # self.embed_to_hidden_weight = nn.init.xavier_uniform(torch.tensor(embeddings))
        # self.embed_to_hidden_bias = nn.init.uniform(torch.tensor(embeddings))
        #
        # # nn.init.xavier_uniform(self.embed_to_hidden_weight)
        # # nn.init.uniform(self.embed_to_hidden_bias)
        #
        # self.dropout = nn.Dropout(p=self.dropout_prob)
        #
        # self.hidden_to_logits_weight = nn.init.xavier_uniform(torch.tensor(embeddings))
        # self.hidden_to_logits_bias = nn.init.uniform(torch.tensor(embeddings))
        # nn.init.xavier_uniform(self.hidden_to_logits_weight)
        # nn.init.uniform(self.hidden_to_logits_bias)
    ...
    def forward(self, w):
        ...
        x_W = torch.matmul(embeddings, self.embed_to_hidden_weight)
        h = F.relu(x_W + self.embed_to_hidden_bias)
        h_U = torch.matmul(h, self.hidden_to_logits_weight)
        logits = h_U + self.hidden_to_logits_bias

        #第二种方法，利用nn.Linear方法表示神经元和权重矩阵的计算
        # input_to_neural = nn.Linear(self.embed_size * self.n_features, self.hidden_size)
        # hidden = F.relu(input_to_neural(embeddings))
        # hidden_drop = self.dropout(hidden)
        # neural_to_output = nn.Linear(self.hidden_size, self.n_classes)
        # logits = neural_to_output(hidden_drop)
