...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        ls_transitions = model.predict(minibatch)
        for parse, transition in zip(minibatch, ls_transitions):
          parse.parse_step(transition)
          if (len(parse.buffer) == 0 and len(parse.stack) == 1): unfinished_parses.remove(parse)
