...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            raise ValueError("transition made is invalid, choose 'S' 'LA' or 'RA' instead")
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        unfinished_parses_temp = []
        for parse in unfinished_parses:
            if not(len(parse.buffer)==0 and len(parse.stack)==1):
                unfinished_parses_temp.append(parse)
        unfinished_parses = unfinished_parses_temp
